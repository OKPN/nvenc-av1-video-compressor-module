...
5. **厳密ソケットバインド判定 ＆ ポート競合回避アルゴリズム**:
   - `SO_REUSEADDR` を有効化した厳密ソケットチェックにより、再起動直後の `TIME_WAIT` ポートを即座に再利用可能。初期ポート `7861` が占有されている場合は、自動で `7862` ➔ `7863` ... と空きポートを探索して起動します。
6. **UI からのワンクリック再起動 (`restart_app`)**:
   - 「⚙️ 設定」タブの `♻️ アプリを再起動` ボタンを押すだけで、設定の保存とプロセスの完全置換再起動を全自動で行えます。
7. **ポータブル＆簡単なアンインストール**:
...
4. Web UI アプリの自動起動 ＆ ブラウザ自動オープン (http://localhost:7861)
```

起動後、自動でブラウザが開いて `http://localhost:7861` にアクセスします。（※ポート `7861` が占有されている場合は `7862`, `7863` ... と自動で+1ずつ空きポートを探して起動します）
...
不要になった場合は、**本アプリのフォルダ全体をそのまま手動で削除（ごみ箱へ移動）するだけ** でアンインストールが完了します。

*(※ 自動インストールされた FFmpeg もシステムから残さず消したい場合は、気になるなら**アプリ内の「⚙️ 設定」タブから「🗑️ アンインストール (Winget)」を押して FFmpeg を削除してから、本フォルダを削除** してください)*
